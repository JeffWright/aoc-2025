...
fun day(day: Int, part: Int, block: suspend (String) -> String) {
  runBlocking {
    AdventOfCodeClient().use { client ->
      val input = client.fetchInput(2025, day)
      val answer: String
      val time = measureTime { answer = block(input) }
...
fun sample(input: String, correctAnswer: String, block: suspend (String) -> String) {
  runBlocking {
    val answer: String
    val time = measureTime { answer = block(input) }
